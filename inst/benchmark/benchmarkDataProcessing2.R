...
mean(SERGIO != 0)
corValues <- as.matrix(scTenifoldNet::pcNet(as.matrix(SERGIO), symmetric = FALSE, nComp = 10))
for(i in seq_len(nrow(corValues))){
  for(j in seq_len(ncol(corValues))){
    if(j > i){
      if(abs(corValues[i,j]) > abs(corValues[j,i])){
        corValues[j,i] <- 0
      } else {
        corValues[i,j] <- 0
      }
...
png('fig1_dirPCR.png', width = 2300, height = 2000, res = 300)
