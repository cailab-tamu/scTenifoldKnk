...
library(pbapply)
nGenes <- pbsapply(seq_len(100)[-1], function(Z){
  dGenes <- lapply(allDatasets, function(X){
    ...
    dGenes[!grepl('^Rpl|^Rps|^mt-', dGenes, ignore.case = TRUE)]
  })
...
nGenes <- nGenes[-3,]
which.max(rowMeans(apply(nGenes,1,rank))) + 1
...
dGenes <- lapply(allDatasets, function(X){
  ...
  PA <- p.adjust(PV, method = 'fdr')
  dGenes <- X$diffRegulation$gene[PA < 0.05]
  dGenes <- dGenes[!grepl('^Rpl|^Rps|^mt-', dGenes, ignore.case = TRUE)]
